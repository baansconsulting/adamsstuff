...
{		
		...
		else 
		{
			...
			if($_FILES["file"]["size"] > 0)
			{
				...
				if($flag)
				{
					$query="SELECT `date_range`, `parent_asin`, `child_asin`, `title`, `sku`, `sessions`, `session_percentage`, `page_views`, `page_views_percentage`, `buy_box_percentage`, `units_ordered`, `units_ordered_b2b`, `unit_session_percentage`, `unit_session_percentage_b2b`, `ordered_product_sales`, `ordered_product_sales_b2b`, `total_order_items`, `total_order_items_b2b` FROM `tbltempdashboard` WHERE user_id=".$user_id;
					...
					// $masterInsert=$insert.rtrim($masterInsert,",");
					// echo $masterInsert;
					//$R = mysql_query($query);
					echo '<script language="javascript">
					...
					//update sku master
					$Q = "insert into tblskumaster(sku,skuname) SELECT tbldashboard.sku,concat(tbldashboard.sku,'_Name') as skuname FROM tbldashboard where tbldashboard.sku not in(select tblskumaster.sku from tblskumaster where  tblskumaster.sku=tbldashboard.sku)";
					$R = mysql_query($Q);
					//if($R)
					//{
		...
		<center style="background-color: lightgray;">
			<table style="margin-top: 40px;width: 16%;text-align: center;"><tr>
				...
				<td><button onclick="deleteCSV('<?php echo $filename; ?>',1);" style="">Delete</button></td>
			</tr>
			</table>
		</center>
